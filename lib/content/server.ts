...
export async function getContent(): Promise<{settings:typeof settings;projects:Project[];stats:typeof stats;journey:Entry[];education:Entry[];skillGroups:Record<string,string[]>}>{
 const fallback={settings,projects,stats,journey,education,skillGroups};const id=process.env.NEXT_PUBLIC_SANITY_PROJECT_ID,dataset=process.env.NEXT_PUBLIC_SANITY_DATASET;if(!id||!dataset)return fallback;
 ...
 const query=`{"projects":*[_type=="project"]|order(order asc){...,"slug":slug.current,"image":cover.asset->url,"github":links.github,"live":links.live},"stats":*[_type=="stat" && defined(sourceNote)]|order(order asc){value,label,sourceNote},"journey":*[_type=="experience"]|order(order asc){"role":position,"organization":company,period,location,"text":description,"bullets":responsibilities,"techs":technologies},"education":*[_type=="education"]|order(order asc){"role":degree,"organization":institution,period,location,"text":description,"bullets":responsibilities,"techs":technologies},"skillCategories":*[_type=="skillCategory"]|order(order asc){title,tools}}`;
 try{
