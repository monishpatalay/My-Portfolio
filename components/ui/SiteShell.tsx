...
export default function SiteShell({children}:{children:React.ReactNode}){const path=usePathname(),[entryPath]=useState(path);if(path.startsWith('/studio'))return children;return <MotionRoot>{entryPath==='/'&&<Preloader/>}<CursorGlow/><Nav/>{children}<Footer/><Chat/><FullscreenNotice/></MotionRoot>;}
